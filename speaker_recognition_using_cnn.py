...
"""
from components import EntropyPoolLayer, EntropyPooling1D
...
def build_model(input_shape, num_classes):
    ...
    # print(f"input to pooling {tf.shape(x)}")
    # average_pooling1d(AveragePooli(None, 83, 128)    0    max_pooling1d_4[0][0]
    x = keras.layers.AveragePooling1D(pool_size=3, strides=3)(x)
    # print(f"output of pooling {tf.shape(x)}")
    # x = EntropyPooling1D(pool_size=3, strides=3)(x)
